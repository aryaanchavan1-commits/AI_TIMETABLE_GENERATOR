...
class AITimetableGenerator:
    ...
    def research_task_importance(self, task_name: str, task_description: str = "") -> Dict[str, Any]:
        """
        Research task importance using AI analysis
        Analyzes task characteristics to determine optimal importance level
        """
        if not self.client:
            return {
                'importance': 'medium',
        ...
        try:
            prompt = f"""
            Analyze the following task and determine its importance level based on:
            1. Task type and nature
            2. Common productivity patterns
            3. Typical urgency indicators
            4. Impact on goals and deadlines
            
            Task: {task_name}
            Description: {task_description}
            
            Consider these factors:
            - Work-related tasks (meetings, deadlines, projects) are typically HIGH importance
            - Learning and skill development are MEDIUM to HIGH importance
            - Administrative tasks are typically MEDIUM importance
            - Personal care and exercise are MEDIUM importance
            - Social and entertainment are typically LOW importance
            - Tasks with deadlines are higher importance
            - Tasks that affect others are higher importance
            ...
            {{
                ...
                "suggestions": ["List of productivity suggestions"],
                "optimal_time": "Best time of day for this task",
            ...
            chat_completion = self.client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a productivity analysis expert. Analyze tasks and provide importance ratings with confidence scores."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def analyze_user_behavior(self, 
        ...
        """
        Analyze user behavior patterns to improve task scheduling
        """
        if not self.client:
            return {
                'peak_hours': ['09:00-12:00', '16:00-18:00'],
        ...
        try:
            prompt = f"""
            Analyze the user's task completion patterns and provide insights:
            
            Completed Tasks (last 7 days):
            {json.dumps(completed_tasks[-20:], indent=2)}
            ...
            {{
                "peak_hours": ["List of most productive time ranges"],
                "productivity_patterns": ["Observed patterns in task completion"],
            ...
            chat_completion = self.client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a behavioral analyst specializing in productivity patterns. Analyze user behavior and provide actionable insights."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def auto_detect_importance(self, 
                              tasks: List[Dict[str, Any]], 
                              user_history: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        Automatically detect importance level for tasks using AI research
        """
        enhanced_tasks = []
        
        for task in tasks:
            # Research task importance
            research_result = self.research_task_importance(
                task.get('name', ''),
                task.get('description', '')
            )
            
            # Enhance task with research results
            enhanced_task = task.copy()
            enhanced_task['importance'] = research_result.get('importance', 'medium')
            enhanced_task['ai_confidence'] = research_result.get('confidence', 0.5)
            enhanced_task['ai_reasoning'] = research_result.get('reasoning', '')
            enhanced_task['ai_suggestions'] = research_result.get('suggestions', [])
            enhanced_task['optimal_time'] = research_result.get('optimal_time', '')
            enhanced_task['energy_level'] = research_result.get('energy_level', 'moderate')
    ...
    def _create_user_context(self, 
                            ...
                            behavior_analysis: Dict[str, Any] = None) -> str:
        ...
        if behavior_analysis:
            ...
            Peak Hours: {', '.join(behavior_analysis.get('peak_hours', []))}
            Productivity Patterns: {', '.join(behavior_analysis.get('productivity_patterns', []))}
            Energy Curve: {json.dumps(behavior_analysis.get('energy_curve', {}))}
            Recommendations: {', '.join(behavior_analysis.get('recommendations', []))}
            """
        ...
        USER TASKS (Sorted by Importance with AI Analysis):
    ...
    def generate_timetable(self, 
                          ...
                          user_history: List[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Auto-detect importance if enabled
            if auto_detect:
                tasks = self.auto_detect_importance(tasks, user_history)
            
            # Analyze user behavior if history is available
            behavior_analysis = None
            if user_history:
                behavior_analysis = self.analyze_user_behavior(user_history, {})
            
            user_context = self._create_user_context(tasks, goals, preferences, current_time, behavior_analysis)
